...
function getCountryByName(country) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const response = yield fetch(`${API_URL}/rest/v2/name/${country}?fields=name;borders;region;capital`);
        return yield response.json();
    });
}
function getCountryCapitalsByCodes(codes) {
    return __awaiter(this, void 0, void 0, function* () {
        if (codes.length == 0) {
            return [];
        }
        const codeString = codes.join(';');
        const response = yield fetch(`${API_URL}/rest/v2/alpha?codes=${codeString}`);
        const json = yield response.json();
        const capitals = json.map((country) => country.capital);
        return capitals;
    });
}
/*
...
*/
function getCapitalNeighbours(country, codeMap) {
    return __awaiter(this, void 0, void 0, function* () {
        const neighbourCountries = country.borders.map((code) => codeMap[code]);
        const capitals = yield getCountryCapitalsByCodes(country.borders);
        capitals.push(country.capital);
        return capitals;
    });
}
// Populate element cointainer with list of capitals of neighbouring countries inclusive
function requestAndPopulateNeighbourCapitals(event) {
    return __awaiter(this, void 0, void 0, function* () {
        event.preventDefault();
        // Get input from input text
        const countryInput = document.getElementById('input-text-country');
        ...
        const matchingCountries = yield getCountryByName(countryName);
        ...
        const capitals = yield getCapitalNeighbours(country, codeMap);
        // Display results to DOM
        populateNeighbourCapitals(capitals);
    });
}
function populateNeighbourCapitals(capitals) {
    const ul = document.getElementById('display-countries');
    // Clear container before filling in
    ul.innerHTML = '';
    capitals.forEach((capital) => {
        const li = document.createElement('li');
        const liText = document.createTextNode(capital);
